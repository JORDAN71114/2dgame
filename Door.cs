...
public class Door : MonoBehaviour
{
	...
	public void DecrementCollectibles()
	{
		...
		if (collectiblesCount == 5)
		{
			...
			winText.text = "Cakes Collected! Get to the door!";
		}
	}
	...
	void OnTriggerEnter2D(Collider2D target)
	{
		...
		if (target.tag == "Player" & collectiblesCount == 5)
			{
			//if a collision with the player occurs, do this.  This can be used to load the next scene.
			Debug.Log("Level Finished");  //say level finished in the console
			nextlevel.gameObject.SetActive(true); //the next level button appears
